...
export async function GET(req: Request, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  try {
    ...
    if (!job) {
      return errorResponse('RESOURCE_NOT_FOUND', 'Scan job not found.', 404);
    }

    // Access control: students can only view their own jobs
    ...
    console.error('Error fetching port scan status:', err);
